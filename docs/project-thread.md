# Project context

Project Thread is a standalone skill bundle for engineering continuity and native iOS client development. It has no application runtime or backend.

| Authority | Location |
| --- | --- |
| Product scope, public skills, installation | [README](../README.md) |
| Bundle architecture and ownership | [Architecture](architecture.md) |
| Validation commands and evidence limits | [Testing](testing.md) |
| Shared priorities and cross-area dependencies | [Roadmap index](roadmap.md) |
| Bundle outcomes and delivery records | [Skill-suite roadmap](roadmaps/skill-suite/README.md) |
| Candidate application evaluation | [iOS-adoption roadmap](roadmaps/ios-adoption/README.md) |
| Current implementation plan | [Area and initiative roadmaps](roadmaps/skill-suite/plans/area-roadmaps.md) |
| Current phase and evidence | [Integrate roadmap records](roadmaps/skill-suite/phases/area-roadmaps/01-integrate-records.md) |
| Current work item | [Roadmap record ownership](roadmaps/skill-suite/issues/roadmap-record-ownership.md) |
| Completed foundation | [Engineering and iOS v1](plans/engineering-ios-v1.md) |
| Current continuation state | [Checkpoint](checkpoints/current.md) |
| Shared adopter records | [Record ownership](../skills/project-thread/references/records.md) |
| Decision index | [Agent Notes](../.agents/notes/README.md) |
| Evaluation inputs and method | [Behavioral fixtures](../evals/README.md) |

Run commands from the repository root. The primary check is `python3 scripts/check.py`; helper regressions use `python3 -m unittest discover -s tests -v`. iOS application commands belong in each adopting project's testing reference and cookbooks.
