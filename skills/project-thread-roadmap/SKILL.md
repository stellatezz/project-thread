...
Read current business goals, constraints, existing plans, and the [record ownership rules](../project-thread/references/records.md). Identify the smallest valuable outcome before ordering later capabilities. A demanding reference product demonstrates workflows, not a requirement to copy its infrastructure.
...
Update the owning roadmap from plan/phase evidence using [phase execution](../project-thread-phase/SKILL.md). Update affected issue, dependency, and index summaries when readiness or shared priority changes; keep detailed execution state in its plan/phase owner. Record changed rationale in [Agent Notes](../project-thread-agent-notes/SKILL.md), and identify the roadmap, plan, phase, and relevant issue blockers in the checkpoint.
