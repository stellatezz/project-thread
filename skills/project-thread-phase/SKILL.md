...
Recover the active phase file or plan section, its parent plan and roadmap, linked issues/decisions, and [checkpoint](../project-thread/references/checkpoints.md); inspect the actual working tree and relevant records. Preserve unrelated changes. Confirm the next phase's scope and unresolved dependencies without re-requesting settled authorization. Use [delivery-record guidance](../project-thread/references/delivery-records.md) to keep phase evidence and issue state with their owners.

Apply [engineering](../project-thread-engineering/SKILL.md), plus [iOS](../project-thread-ios/SKILL.md) for native iOS work. Use established repository patterns and cookbooks. Implement the phase through the real application entry path, including the accepted failure and recovery states.
...
Follow [record ownership](../project-thread/references/records.md) for completion. Update affected current docs, the owning [Agent Note](../project-thread-agent-notes/SKILL.md), useful learned cookbooks, plan and owning roadmap evidence, and the checkpoint. Reflect changed cross-area dependency readiness in linked roadmaps and the index; consuming phases still require their own acceptance evidence. A required simulator, device, migration, or integration check that has not run remains open. Report implemented behavior, checks, remaining gaps, and the next action.
